...
export default function NewBookingPage() {
  ...
  const { state } = useLocation() as { state:{room:RoomResponseDto;checkIn:string;checkOut:string} };
  const { createBooking, loading, error } = useBookingController();
  const [notes, setNotes] = useState("");

  if (!state?.room) { navigate("/rooms"); return null; }
  ...
  const handleConfirm = async () => {
    ...
    const booking = await createBooking({ guestId:user.id, roomId:room.id, checkInDate:checkIn, checkOutDate:checkOut, notes:notes||undefined });
    if (booking) navigate(`/bookings/${booking.id}/pay`, { state:{booking,total} });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="bg-bg-white border border-bg-card p-8">
            ...
            <div className="space-y-5">
              {[
                { label:"Habitación",       value:`${room.number} — ${RoomTypeLabel[room.type]}` },
                { label:"Check-In",         value:checkIn   },
                { label:"Check-Out",        value:checkOut  },
                { label:"Noches",           value:`${nights}` },
                { label:"Tarifa por noche", value:`$${room.basePrice}` },
              ].map(row => (
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-[10px] tracking-[0.2em] uppercase text-text-main font-medium">Total</span>
            ...
            <div className="mt-8 bg-bg-base p-4">
              <p className="text-[10px] tracking-wide text-text-sub leading-relaxed">
                ...
                ✦ Confirmación inmediata por correo electrónico<br />
                ✦ Desayuno gourmet incluido en suites
              </p>
            </div>
          </div>
